...
export const ethereum = {
  ...
  init: (path = 'ethereum,1', keyVersion = 0) => {
    ethereum.path = path;
    ethereum.keyVersion = keyVersion;
    return ethereum;
  },

  getAddress: () => genAddress('ethereum'),
  ...
  completeEthereumTx: async ({ address, baseTx }) => {
    ...
    if (NEAR_PROXY_CONTRACT === 'true') {
      sig = await near.sign(unsignedTx, MPC_PATH);
    } else {
      sig = await near.sign(payload, MPC_PATH);
      // payload was reversed in near.sign(...) call for MPC contract, reverse it back to recover eth address
      payload.reverse();
    }
    if (!sig) return;
